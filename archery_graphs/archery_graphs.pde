...
void setup() {
  ...
  size_w = (width - 140)/2;
  size_h = (height - 200 )/2;
  canvas[0] = new subCanvas(width/4, height/4, 5, size_h, size_h);
  canvas[1] = new subCanvas(3*width/4, height/4, 5, size_w, size_h);
  canvas[2] = new subCanvas(width/4, 3*height/4, 5, size_h, size_h);
  canvas[3] = new subCanvas(3*width/4, 3*height/4, 5, size_h, size_h);
  
  bow = new Bow(diameter);
}
...
void draw() {
  ...
  canvas[2].showAxis("BOW1 POSITION (X-Y GRAPH)");
  canvas[3].showAxis("BOW2 POSITION (Z-Y GRAPH)");
  ...
  //loading csv file
  Table table = loadTable("data/data.csv");
  try{
    TableRow row = table.getRow(0);
  
    bary = new Barycenter(row.getFloat(0), row.getFloat(1), diameter);
    bow.append_to_array(row.getFloat(2), row.getFloat(3), row.getFloat(4));
  ...
  bary.display(width / 4, height / 4, size_h - 20, size_h - 20);  // to make sure points are never going to be on the border of the window
  bow.display(width / 4, 3 * height / 4, size_h - 20, size_h - 20, false);
  archer.display(3 * width / 4, height / 4, size_w - 20, size_h - 20, max_x_length, max_y_length);  
