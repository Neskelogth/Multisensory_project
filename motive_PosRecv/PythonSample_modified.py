...
import socket
import time
from NatNetClient_Aneek_mod import NatNetClient
...
def print_commands(can_change_bitstream):
    ...
    outstring += "q  quit\n"
    outstring += "\n"
    outstring += "NOTE: MotiveHERE frame playback will respond differently in\n"
...
if __name__ == "__main__":
    ...
    #streaming_client.set_print_level(1000)



    IP_UserComm = '127.0.0.1'
    Port_UserComm = 8765

    sokcet_UserComm = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sokcet_UserComm.bind((IP_UserComm, Port_UserComm))
    sokcet_UserComm.settimeout(0.1)
    ...
    while is_looping:
        ...
        try:
            ...
            #recvline,addr = sokcet_UserComm.recvfrom(4096)
            #inchars=recvline.decode()

            #######################################
            ##### ACEPT UDP COMMAND OR KEY-IN #####
            inchars = ''
            if msvcrt.kbhit():
                inchars = read_input_my()
