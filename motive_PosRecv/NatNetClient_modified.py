...
###########################################
##### Data for Mocap2Controll (T.Ito) #####

def send_data_to_M2C(self, data):  
    data_bin = p.dumps(data)  
    self.socket_M2C.sendto(data_bin, (self.IP_M2C, self.Port_M2C))
...
class NatNetClient:
    ...
    def __init__( self ):
        ...
        #############################################
        ##### socket for Mocap2Controll (T.Ito) #####

        self.IP_M2C = "127.0.0.1"
        self.Port_M2C = 1234
        self.socket_M2C = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    def __unpack_labeled_marker_data( self, data, packet_size, major, minor):
        ...
        if( ( major == 2 and minor > 3 ) or major > 2 ):
            ...
            for _ in range( 0, labeled_marker_count ):
                ...
                pos = Vector3.unpack( data[offset:offset+12] )
                #HERE
#                print("types here+",type(model_id),type(marker_id),type(pos),type(pos[0]))
#                print("data+",model_id,marker_id,pos[0],pos[1],pos[2])
                to_send = [model_id,marker_id,pos[0],pos[1],pos[2]]
#
                send_data_to_M2C(self, to_send)
#               
                offset += 12
    ...
    def __command_thread_function( self, in_socket, stop, gprint_level):
        ...
        while not stop():
            ...
            #print(data,"NNC line 1189, len(data)")
            ...
            if len( data ) > 0 :
                ...
                print(message_id," NNC line 1205")
                data=bytearray(0)
